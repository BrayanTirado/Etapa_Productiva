...
def enum_choices(model, column_name: str):
    ...
    return None  # Retorna None si no es Enum
...
def listar_programas():
    programas = []
    aprendiz_id = None  # Para el botón Volver

    # Aprendiz ve su propio programa
    ...
    # Instructor selecciona aprendiz
...
def nuevo_programa():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.flush()  # Para que tenga id_programa

            # Vincular aprendiz actual al programa recién creado si aplica
            if isinstance(current_user, Aprendiz):
                current_user.programa = nuevo
...
def editar_programa(id):
    ...
    # Solo el aprendiz dueño puede editar
    if not (isinstance(current_user, Aprendiz) and current_user.programa == programa):
        ...
        return redirect(url_for('programa_bp.listar_programas', aprendiz_id=getattr(programa.aprendiz, 'id_aprendiz', None)))
    ...
    if request.method == 'POST':
        ...
        if not nombre:
            ...
            return render_template('programa/editar_programa.html',
                                   programa=programa, titulo=titulo_ops, jornada=jornada_ops, centro_formacion=centro_ops)
...
def eliminar_programa(id):
    ...
    # Solo el aprendiz dueño puede eliminar
    if not (isinstance(current_user, Aprendiz) and current_user.programa == programa):
